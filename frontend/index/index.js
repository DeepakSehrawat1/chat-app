...
async function handleGroupNameClick(groupId, groupName) {
  try {
    ...
    const response = await axios.get(
      `http://:localhost3000/group/getgroupmessages/${groupId}`,
      { headers: { Authorization: token } }
    );
    ...
    if (response.status === 200) {
      ...
      throw new Error(
        `Failed to get group messages. Status: ${response.status}`
      );
    }
  } catch (err) {
    if (
      err.response &&
      err.response.data &&
      err.response.data.err === "No messages found for the group"
    ) {
      const parentElement = document.getElementById("msg");
...
async function loadUserList(groupId) {
  try {
    ...
    const responseUsers = await axios.get(
      `http://localhost:3000/group/getgroupusers/${groupId}`,
      { headers: { Authorization: token } }
    );
    ...
    if (responseUsers.status === 200) {
      ...
      userListContainer.innerHTML = `<h4>Users in this group: ${groupUsers
        .map((user) => {
      ...
      throw new Error(
        `Failed to get group users. Status: ${responseUsers.status}`
      );
    }
  } catch (err) {
    showError(err);
...
async function addUsertoGroup(e) {
  try {
    ...
    const userName = window.prompt(
      `Enter the user's name to add to ${currentGroup.name}:`
    );
    ...
    if (userName !== null && userName.trim() !== "") {
      ...
      const response = await axios.post(
        `http://localhost:3000/group/adduser/${groupId}`,
        { userName },
        { headers: { Authorization: token } }
      );
...
async function removeUserfromGroup(e) {
  try {
    e.preventDefault();
    const userName = window.prompt(
      `Enter the user's name to remove from ${currentGroup.name}:`
    );
    console.log(userName);
    if (userName !== null && userName.trim() !== "") {
      // If a name is provided, make a request to the backend to add the user
      ...
      const response = await axios.post(
        `http://localhost:3000/group/removeuser/${groupId}`,
        { userName },
        { headers: { Authorization: token } }
      );
...
async function loadOlderChats() {
  try {
    ...
    if (currentGroup) {
      const response = await axios.get(
        `http://localhost:3000/message/getoldmessage?firstmessageid=${firstmessageid}&groupId=${currentGroup.id}`,
        { headers: { Authorization: token } }
      );
...
async function sendmsg(e) {
  try {
    ...
    if (response.status === 201) {
      ...
      const parentElement = document.getElementById("msg");
      addMessageToUI(response.data.message, parentElement);
      msgBox.value = "";
    } else {
    ...
    if (
      err.response &&
      err.response.data &&
      err.response.data.err === "No message to send"
    ) {
      errorBox.innerHTML = "";
...
async function sendimg(e) {
  try {
    ...
    const response = await axios.post(
      "http://localhost:3000/message/sendimage",
      formData,
      {
        headers: {
