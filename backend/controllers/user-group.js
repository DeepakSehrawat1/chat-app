...
const createGroup = async (req, res) => {
  ...
  try {
    ...
    const userGroupMappings = await Promise.all(
      participantUserIds.map(async (user) => {
    ...
    return res
      .status(200)
      .json({ group: createdGroup, userGroupMappings: userGroupMappings });
  } catch (err) {
    await t.rollback();
...
const getGroupUsers = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to retrieve users in the group" });
  }
};
