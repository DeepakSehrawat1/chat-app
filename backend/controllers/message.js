...
const sendimage = async (req, res) => {
  ...
  try {
    ...
    global.socketServer.to(groupId).emit("newImage", createdMsg);
...
const sendmessage = async (req, res) => {
  ...
  try {
    ...
    if (!isUserInGroup) {
      return res
        .status(403)
        .json({ err: "User is not a member of the specified group" });
    }
    ...
    global.socketServer.to(groupId).emit("newMessage", createdMsg);
